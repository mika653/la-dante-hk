        ...
        <ul className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {items.map(({ icon: Icon, label, detail }) => (
            <li key={label} className="flex items-center gap-3">
              <span className="w-10 h-10 rounded-full bg-cream-2 text-azzurro inline-flex items-center justify-center shrink-0 border border-line"><Icon size={18} aria-hidden /></span>
              <div>

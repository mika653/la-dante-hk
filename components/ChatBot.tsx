  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 w-14 h-14 md:w-16 md:h-16 rounded-full bg-azzurro text-cream shadow-[var(--shadow-pop)] hover:scale-105 transition-transform flex items-center justify-center"
      >
      ...
      {open && (
        <div className="fixed bottom-24 right-4 md:right-6 z-50 w-[calc(100vw-2rem)] md:w-[380px] h-[500px] frame bg-white flex flex-col overflow-hidden animate-in">
          <header className="bg-azzurro text-cream p-4 flex items-center gap-3">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-cream">
            {msgs.map((m, i) => (
              <div key={i} className={`flex gap-2 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                <span className={`w-7 h-7 rounded-full inline-flex items-center justify-center shrink-0 ${m.role === "bot" ? "bg-azzurro text-cream" : "bg-sole text-ink"}`}>
                  {m.role === "bot" ? <Bot size={14} aria-hidden /> : <UserIcon size={14} aria-hidden />}
                </span>
                <div className={`max-w-[80%] rounded-2xl px-3.5 py-2.5 text-sm ${m.role === "bot" ? "bg-white border border-line" : "bg-azzurro text-cream"}`}>
          ...
          <form onSubmit={(e) => { e.preventDefault(); send(); }} className="p-3 border-t border-line flex gap-2 bg-white">
            <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask about courses, prices, PLIDA..." className="flex-1 h-11 px-4 rounded-full border border-line bg-white focus:outline-none focus:border-azzurro text-sm" />
            <button type="submit" aria-label="Send" className="w-11 h-11 rounded-full bg-azzurro text-cream inline-flex items-center justify-center disabled:opacity-40" disabled={!input.trim()}>
              <Send size={16} />
            </button>
          </form>
        </div>

...
import { ArrowLeft, Plus, Trash2, Check, RotateCcw, CalendarDays, GraduationCap, X } from "lucide-react";
import { useClosures } from "@/lib/use-closures";
import { addHoliday, addPlidaDate, removeClosure, resetHolidays, resetPlidaDates } from "@/lib/closures-actions";
...
function errText(e: unknown) {
  ...
  return /Not authorised/i.test(m) ? "You need to be signed in as an owner or manager to edit these. Please sign in first." : m;
}

export default function HolidaysPage() {
  const { holidays, plida, reload } = useClosures();
  ...
  async function add() {
    ...
    await run(() => addHoliday({ date, name: name.trim(), ...(hasRange ? { endDate } : {}) }), `Added ${name.trim()}.`);
    setDate(""); setEndDate(""); setName("");
  }
  async function resetSeed() {
    if (!confirm("Reset to the built-in Hong Kong holidays? Everyone's custom edits will be replaced.")) return;
    await run(() => resetHolidays(), "Reset to the built-in Hong Kong holidays.");
  }

  // ---- PLIDA exam days ----
  async function addPlida() {
    if (!pDate || !pName.trim()) return;
    await run(() => addPlidaDate({ date: pDate, name: pName.trim() }), `Added ${pName.trim()}.`);
    setPDate(""); setPName("");
  }
  async function resetPlida() {
    if (!confirm("Reset to the default PLIDA sittings? Everyone's custom edits will be replaced.")) return;
    await run(() => resetPlidaDates(), "Reset to the default PLIDA dates.");
  }
  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="mt-2 text-ink-muted">These are the days the <b>course scheduler skips</b> when generating lesson dates. They&apos;re <b>shared</b> — edit them once and everyone in the office sees the same list. Public holidays and school closures always skip; PLIDA exam days only skip for classes you tick <b>&ldquo;Skip PLIDA&rdquo;</b> on.</p>
      ...
      {err && (
        <div className="mt-4 frame p-3 bg-rosso/10 text-rosso flex items-start gap-2 text-sm">
      ...
      <div className="mt-6 frame p-5 md:p-6 bg-white">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <button type="button" onClick={add} disabled={busy || !date || !name.trim()} className="btn btn-primary h-11 disabled:opacity-40"><Plus size={16} /> Add</button>
        </div>
      </div>
      ...
      <div className="mt-6 frame bg-white overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 bg-cream-2 border-b border-line">
          ...
          <button type="button" onClick={resetSeed} disabled={busy} className="text-xs text-ink-muted hover:text-azzurro-deep inline-flex items-center gap-1 disabled:opacity-40"><RotateCcw size={12} /> Reset to HK holidays</button>
        </div>
        <ul>
          ...
          {sorted.map((h) => (
            <li key={h.id ?? h.date} className="flex items-center gap-4 px-5 py-3 border-b border-line last:border-b-0">
              ...
              <button type="button" onClick={() => h.id && run(() => removeClosure(h.id!), "Removed.")} disabled={busy || !h.id} title="Remove" className="w-8 h-8 rounded-lg hover:bg-rosso/10 hover:text-rosso inline-flex items-center justify-center text-ink-muted disabled:opacity-40"><Trash2 size={14} /></button>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="mt-10 frame p-5 md:p-6 bg-white">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <button type="button" onClick={addPlida} disabled={busy || !pDate || !pName.trim()} className="btn btn-primary h-11 disabled:opacity-40"><Plus size={16} /> Add</button>
        </div>
      </div>
      ...
      <div className="mt-6 frame bg-white overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 bg-cream-2 border-b border-line">
          ...
          <button type="button" onClick={resetPlida} disabled={busy} className="text-xs text-ink-muted hover:text-azzurro-deep inline-flex items-center gap-1 disabled:opacity-40"><RotateCcw size={12} /> Reset to defaults</button>
        </div>
        <ul>
          ...
          {plidaSorted.map((p) => (
            <li key={p.id ?? p.date} className="flex items-center gap-4 px-5 py-3 border-b border-line last:border-b-0">
              <span className="font-mono text-xs text-azzurro-deep w-40 shrink-0">{fmt(p.date)}</span>
              <span className="flex-1 text-sm">{p.name}</span>
              <button type="button" onClick={() => p.id && run(() => removeClosure(p.id!), "Removed.")} disabled={busy || !p.id} title="Remove" className="w-8 h-8 rounded-lg hover:bg-rosso/10 hover:text-rosso inline-flex items-center justify-center text-ink-muted disabled:opacity-40"><Trash2 size={14} /></button>
            </li>
          ))}
        </ul>
      </div>

      <p className="mt-3 text-xs text-ink-muted">Changes save to the shared list and apply to the course scheduler right away, on every computer.</p>
    </div>
  );
}
